#!/usr/bin/env python3
"""
Magic8 Data Processor - Optimized Version
This version addresses memory issues by:
- Writing data incrementally instead of storing everything in memory
- Processing in batches
- Showing progress and performance metrics
"""
...
class Magic8DataProcessorOptimized:
    ...
    def write_batch(self):
        """Write current batch to CSV file."""
    ...
    def save_final_reports(self):
        ...
        strategy_stats = {
            ...
            'by_strategy_pct': {k: round(v/self.total_trades_processed*100, 2) for k, v in self.strategy_counts.items()}
        }
        ...
        for strategy, count in sorted(self.strategy_counts.items()):
            pct = round(count/self.total_trades_processed*100, 2)
...
def main():
    ...
    output_path = "/Users/jt/magic8/magic8-accuracy-predictor/data/processed_optimized"
    
    # Create processor with batch size
    processor = Magic8DataProcessorOptimized(source_path, output_path, batch_size=5000)
    
    # Process all data
    logger.info("Starting Magic8 data processing (optimized version)...")
    logger.info("This version writes data incrementally to avoid memory issues")
