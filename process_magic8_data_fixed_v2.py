#!/usr/bin/env python3
"""
Magic8 Data Processor - Fixed Version V2
This version handles timestamp parsing issues including:
- Form feed characters (\x0c) and other non-printable characters
- Multiple timestamp formats
- Better error handling for individual row failures
"""
...
class Magic8DataProcessorV2:
    ...
    def process_profit_2023_format(self, file_obj, folder_date: datetime, file_path: Path):
        ...
        for row_num, row in enumerate(reader, 2):
            try:
                ...
                logger.debug(f"Error processing row {row_num} in {file_path}: {e}")
    ...
    def parse_trades_timestamp(self, date_str: str, time_str: str) -> Optional[datetime]:
        ...
        try:
            ...
            logger.debug(f"Failed to parse trades timestamp: {date_str} {time_str} - {e}")
            return None
    ...
    def save_processed_data(self):
        ...
        # Save timestamp processing stats
    ...
    def save_timestamp_stats(self):
        """Save timestamp processing statistics."""
        stats = {
            ...
            'success_rate': round((1 - self.timestamp_stats['failed_timestamps'] / max(1, self.timestamp_stats['total_timestamps'])) * 100, 2)
        }
        
        output_file = self.output_path / 'timestamp_processing_stats.json'
        with open(output_file, 'w') as f:
            json.dump(stats, f, indent=2)
        
        logger.info(f"Timestamp Processing Stats:")
        logger.info(f"  Total processed: {stats['total_timestamps_processed']:,}")
        logger.info(f"  Cleaned (had non-printable chars): {stats['timestamps_cleaned']:,}")
        logger.info(f"  Failed to parse: {stats['timestamps_failed']:,}")
...
def main():
    ...
    output_path = "/Users/jt/magic8/magic8-accuracy-predictor/data/processed_fixed_v2"
    
    # Create processor
    processor = Magic8DataProcessorV2(source_path, output_path)
    
    # Process all data
    logger.info("Starting Magic8 data processing (fixed version V2)...")
